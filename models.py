...
class User(db.Model):
	...
	_Results = db.relationship('Result', backref='User', primaryjoin='User.Email==Result.UserEmail' )
	...
	def get_keys(self):
		return ['Email', 'Name', 'Password', 'Theme']
...
class Result(db.Model):
	...
	UserEmail = db.Column(db.String(256), db.ForeignKey('user.Email'), nullable=False )
	...
	def get_keys(self):
		return ['Result_ID', 'UserEmail', 'Value', 'Roman', 'Base_value', 'Date']

	def get_values(self):
		return [ self.UserEmail, self.Value, self.Roman, self.Base_value, self.Date ]
